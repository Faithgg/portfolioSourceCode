let joueur = document.querySelector('div.forms input')
        
let toptop= document.querySelector('button.continuer')
toptop.onclick = () => {
    if (joueur.value.length > 9) {
        let ancienP= document.querySelector('.alertname')
        if (ancienP != undefined) {
            ancienP.remove()
        }
        let alert=document.createElement('p')
        alert.setAttribute('class',"alertname")
        alert.innerText = "9 caractères au maximum s'il vous plait"
        return document.querySelector('.oups').appendChild(alert)
    } else 
   { let premierblock= document.querySelector ("div.premier_block")
    premierblock.setAttribute("style","display:none")
}}
let valeurScore = 0
...
function chargeurDeNiveau() {
    ...
    let champJoueur = document.querySelector("div.joueur p")
        champJoueur.innerHTML='<p><strong>Joueur:</strong><br><em>'+joueur.value+'</em></p>'
...
function findejeu () {
    let hgjklm= document.querySelector ('.buttonApresJeu')
hgjklm.onclick = () => {
    document.querySelector('.conteneur').remove()
    debuter()
    document.querySelector ('.buttonApresJeu').remove()
}
        ...
        if (document.querySelector('div.blocB_enfant') != null)
         {
            ...
            {
                //Collison bloc B    // <-------------------Que faire en cas de collision
                document.querySelector('div.musiquedefond').remove()
                //initialisation du sound
                ...
                let apresJeuRecommencer = document.createElement ('button')
                apresJeuRecommencer.setAttribute ('class',"buttonApresJeu")
                apresJeuRecommencer.innerHTML = "Recommencer"
                document.querySelector('.apresJeu').append(apresJeuRecommencer)
